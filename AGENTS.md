...
- Aunque una etapa siga abierta, si la tarea dejó progreso parcial verificable (código, tests en verde, evidencia nueva) dentro de esa etapa, reflejarlo también en `plan_desarrollo_tp2/README.md`, en una sección breve tipo "Progreso parcial dentro de etapas todavía abiertas" inmediatamente después de la tabla maestra. No alcanza con dejar la nota solo en el `.md` de la etapa: el README es el punto de entrada que un agente o el usuario leen primero, y debe reflejar el estado real sin tener que abrir cada etapa para descubrir avances.
