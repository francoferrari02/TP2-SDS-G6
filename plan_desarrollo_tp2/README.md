...
- **Etapa 7 (relación exploratoria de clusters):** el 31/08 se agregó `python/plot_cluster_order_relationship.py` y se generó `figures/cluster_order_relationship_v1/comparison_va_vs_S_six_densities.png` con nuestros datos finales, ambos modelos, seis densidades y los 14 valores de `eta`. A pedido del usuario, la versión vigente muestra solo los promedios, sin barras de desvío, para reducir la superposición visual; la variabilidad permanece disponible en los CSV fuente. El script valida las `168` combinaciones esperadas y escribe `correlation_summary.csv`. Esta salida reproduce la orientación de ejes de la figura externa (`x=<va>`, `y=<S>`) solo para análisis; no reemplaza la figura obligatoria con `x=<S>`, `y=<va>` y tres densidades, ni resuelve la aclaración docente pendiente sobre el alcance del punto E.
...
- **Etapa 6/7 (ampliación de la grilla de `eta` a 37 puntos, 03/09):** el usuario señaló que la figura `<va>` vs. `eta` (grilla de 14 puntos, paso entero entre `eta=1` y `eta=6`) resultaba poco precisa frente a una figura de referencia externa aceptada por la cátedra en otro grupo, y pidió ampliar la grilla y actualizar tanto Vicsek como el votante. Se decidió agregar paso `0.2` entre `eta=0.6` y `eta=6.2` para `rho=2,4,8` en ambos modelos (23 puntos nuevos; la zona `eta<=0.5` no se tocó, ya estaba densificada), documentado en `DECISIONES_PENDIENTES.md`. Se agregó `python/final_dense_eta_grid_run.py` (generaliza `final_fine_grid_run.py`, semillas deterministas `1600000 + model_offset + rho_offset + 100*eta_index + realization`, sin colisión con lotes previos) y se ejecutaron `2760` corridas nuevas (`2` modelos x `3` rho x `23` eta x `R=20`), `0` fallos, `1476.4s` con `--jobs 10`. Analizado con `pilot_analyze.py`: `2760/2760` `observables.csv` válidos. Validación programática independiente confirmó `138` combinaciones, todas con `R=20`, `steps=3000`, `t_window_start=1500`, `va`/`S` en `[0,1]`. Se actualizaron `python/build_final_vicsek_base_table.py` y `python/build_final_voter_base_table.py` para incorporar este tercer lote de origen y regenerar (mismo nombre de archivo) `final_vicsek_base_grid_steps3000_R20_v1_*` y `final_voter_base_grid_steps3000_R20_v1_*`, ahora con `37` puntos de `eta` (`111` combinaciones cada una, `2220` filas por realización) en vez de `14`. Se re-ejecutó `python/generate_final_figures.py` sin cambios de código y se regeneraron las `36` PNG de `figures/final_production_v1/`: las curvas `<va>`/`<S>` vs. `eta`, antes poligonales entre `eta=1` y `eta=6`, ahora son curvas suaves. También se agregó `python/plot_vicsek_va_vs_eta_paper_style.py`, que replica el estilo visual (colores, marcador por densidad, leyenda sin marco) de la figura de referencia compartida por el usuario, en `figures/vicsek_va_vs_eta_paper_style_v1/`. Alcance: solo `rho=2,4,8` en ambos modelos; el bloque de densidades bajas de clusters sigue con la grilla de 14 puntos original.
...
- **Etapa 7 (fotograma estático Vicsek `rho=2`, `eta=1` vs. `eta=5`, 03/09):** a pedido del usuario se generó una figura de flechas lado a lado para dos casos Vicsek en un tiempo avanzado (`t=2000`, dentro de la ventana estacionaria `t_eq=1500`). Como la matriz final no guarda trayectorias, se re-ejecutaron solo dos realizaciones con `--write-trajectory --trajectory-stride 10`, usando semillas ya trazadas en la tabla final: `eta=1`, `base_seed=922000`, realización `0`; `eta=5`, `base_seed=930000`, realización `0`. Se agregó `python/render_vicsek_rho2_eta1_eta5_snapshots.py`, que lee exclusivamente esos `trajectory.csv`, valida `N=200`, IDs únicos, posiciones en `[0,10]` y `theta` en `[0,2pi)`, y escribe `figures/vicsek_rho2_eta1_eta5_snapshots_v1/vicsek_rho2_eta1_eta5_t2000_side_by_side.png` más un `README.md`. Evidencia: `py_compile` sin errores, `60200` filas por trayectoria, `va(t=2000)=0.927548` para `eta=1` y `0.088270` para `eta=5`; PNG inspeccionado visualmente, con los dos paneles en la misma escala, flechas coloreadas por ángulo y barra angular común. Esto no cierra el punto A: sigue faltando video/GIF o módulo animador final.
...
- **Etapa 9:** se inició el documento colaborativo `Informe TP2` en Google Docs. Incluye carátula e índice, espacios reservados para las secciones a cargo del resto del equipo y la redacción de Polarización con siete figuras insertadas (casos característicos, series temporales, curvas estacionarias y comparación Vicsek-votante). El contenido usa exclusivamente el protocolo final: `rho={2,4,8}`, `N={200,400,800}`, `R=20`, `steps=3000`, `t_eq=1500`, ventana `1500..3000` y desvío estándar entre realizaciones. Verificación estructural: siete objetos de imagen, cero marcadores residuales y títulos nativos para las secciones. La exportación HTML/PDF se generó, pero sus referencias no se materializaron localmente desde el conector; falta la revisión visual página a página y la integración de las secciones, referencias y datos administrativos pendientes.
