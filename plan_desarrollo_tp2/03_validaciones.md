...
- [x] **Vecinos medios compatibles con teoría.**
  - Implementación: `tests/test_mean_neighbors.cpp`, registrado en CTest como `mean_neighbors`. No modifica `tp2::cell_index_neighbors` ni ninguna otra pieza del motor: solo genera posiciones uniformes iniciales (`x, y ~ U[0,L)`, `theta` irrelevante para este test) y mide con el CIM.
  ...
  - Valores medidos (una corrida representativa, `cmake --build build && ./build/test_mean_neighbors`):

    | rho | N | aproximada `rho*pi*rc^2` | finita exacta `(N-1)*pi*rc^2/L^2` | medido | realizaciones | semilla base |
    |---:|---:|---:|---:|---:|---:|---:|
    | 2 | 200 | 6.283 | 6.252 | 6.355 | 40 | 100000 |
    | 4 | 400 | 12.566 | 12.535 | 12.502 | 40 | 200000 |
    | 8 | 800 | 25.133 | 25.101 | 25.123 | 40 | 300000 |
...
- Criterio de consenso: con `eta=0`, `voter_update` nunca crea una orientación nueva, solo copia una ya existente (de la propia partícula o de un vecino). Por lo tanto el conjunto de orientaciones distintas presentes en el sistema nunca puede crecer. Se define **consenso exacto** como que ese conjunto se haya reducido a un único valor (igualdad exacta entre orientaciones, con una tolerancia de `1e-12` que solo absorbe redondeo de `normalize_angle`, no diferencias reales). Esto es más fuerte y más verificable que pedir `va` cercano a 1: `va=1` puede acercarse por redondeo de punto flotante sin que las orientaciones sean exactamente iguales, mientras que "una única orientación distinta" es una propiedad exacta y discreta del propio proceso de copia. La herramienta también informa, por separado, la polarización `va` inicial y final de cada corrida, para distinguir explícitamente entre consenso exacto de orientaciones, polarización cercana a 1 y mera finalización de la corrida.
