#!/usr/bin/env python3
"""Genera polarizacion <v_a> vs ruido eta para Vicsek, estilo "paper" (imagen de referencia).

Lee data/summary/final_vicsek_base_grid_steps3000_R20_v1_by_combo.csv (tabla final
consolidada) y grafica una curva por densidad (rho=2,4,8), con marcador distinto por
densidad, barras de error = desvio estandar entre las R=20 realizaciones
(va_stdev_between_realizations, la definicion de barra ya cerrada para todo el TP en
DECISIONES_PENDIENTES.md), sin titulo interno, leyenda sin marco.

Uso:
    MPLCONFIGDIR=/private/tmp/tp2_mplconfig .venv-mpl311/bin/python \
        python/plot_vicsek_va_vs_eta_paper_style.py
"""
...
OUT_DIR = REPO_ROOT / "figures" / "vicsek_va_vs_eta_paper_style_v1"
...
def main() -> int:
    ...
    rows = read_csv_rows(SUMMARY_DIR / f"{VICSEK_RUN}_by_combo.csv")
    ...
    fig.tight_layout()
    out_path = OUT_DIR / "vicsek_va_vs_eta_paper_style.png"
