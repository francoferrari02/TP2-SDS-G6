#!/usr/bin/env python3
"""<S> vs. ruido eta para Vicsek, las seis densidades juntas (degrade azul->rojo).

Combina en una sola figura las tres densidades base (rho=2,4,8, grilla de 37
puntos de eta) y las tres densidades bajas de clusters (rho=1/pi,1/(2pi),
1/(3pi), grilla de 14 puntos), leyendo las dos tablas finales consolidadas:
...
Ambas comparten protocolo (steps=3000, R=20, t_eq=1500, CIM), a diferencia de
la figura de referencia externa que usa steps distintos por densidad
(2000 pasos para rho>=2, 10000 para rho<1) -- ac' se prefiere mantener un
unico protocolo para las seis curvas, ya cerrado para todo el TP.

Color: degrade azul (densidad mas baja, rho=1/(3pi)) a rojo (densidad mas
alta, rho=8), tomado de la colormap 'coolwarm', para que se lea de un
vistazo el orden de densidades; cada curva ademas tiene un marcador propio
para que las seis sigan siendo distinguibles individualmente. Barras de
error = desvio estandar entre las R=20 realizaciones
...
Uso:
    MPLCONFIGDIR=/private/tmp/tp2_mplconfig .venv-mpl311/bin/python \
        python/plot_vicsek_S_vs_eta_all_densities.py
"""
...
OUT_DIR = REPO_ROOT / "figures" / "vicsek_S_vs_eta_all_densities_v1"

BASE_RUN = "final_vicsek_base_grid_steps3000_R20_v1"
...
def combo_curve(rows, rho_label):
    selected = [r for r in rows if r["model"] == "vicsek" and r["rho_label"] == rho_label]
...
def main() -> int:
    ...
    out_path = OUT_DIR / "vicsek_S_vs_eta_all_densities.png"
