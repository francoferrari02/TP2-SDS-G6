#!/usr/bin/env python3
"""Lanzador de los puntos nuevos de la grilla densa de eta para Vicsek, densidades bajas.

Motivacion: al graficar <S> vs. eta con las seis densidades de Vicsek juntas
(python/plot_vicsek_S_vs_eta_all_densities.py) quedo en evidencia que las
tres densidades bajas de clusters (rho=1/pi,1/(2pi),1/(3pi)) siguen con la
grilla original de 14 puntos de eta, mientras que rho=2,4,8 ya tienen 37
(ver 06_barrido_de_produccion.md, "ampliacion de la grilla de eta a 37
puntos"). Este script corre los mismos 23 puntos nuevos (paso 0.2 entre
eta=0.6 y eta=6.2) para las tres densidades bajas, solo Vicsek.

Mismo protocolo que el resto del barrido final: steps=3000, R=20, CIM, sin
trayectoria. N=32,16,11 (redondeo al entero mas cercano, decision ya
cerrada). Semillas deterministas `2100000 + rho_offset + 100*eta_index +
realizacion`, sin colision con ningun lote previo (el maximo de semilla
reservado hasta ahora era 2060219, en vicsek_va_t_eta_1_3_5_run.py).

Uso:
    python3 python/vicsek_lowrho_dense_eta_grid_run.py [--run-name NOMBRE]
...
# rho_nominal, N, rho_label -- mismo redondeo ya aprobado para clusters bajos.
RHO_GRID = [
...
def main() -> int:
    ...
    parser.add_argument("--run-name", default="vicsek_lowrho_dense_eta_grid_steps3000_R20_v1")
