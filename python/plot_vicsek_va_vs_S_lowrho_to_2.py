#!/usr/bin/env python3
"""<va> vs. <S> para Vicsek, densidades desde la mas baja hasta rho=2.

Combina rho={1/(3pi),1/(2pi),1/pi,2} (grilla de 37 puntos de eta cada una),
leyendo las tablas finales consolidadas:

    data/summary/final_lowrho_cluster_grid_steps3000_R20_v1_by_combo.csv
    data/summary/final_vicsek_base_grid_steps3000_R20_v1_by_combo.csv (solo rho=2)

Cada punto es un eta del barrido; eta no es un eje. Color: degrade de
violeta, mas claro para la densidad mas baja (rho=1/(3pi)) y mas oscuro para
la mas alta (rho=2), con marcador propio por densidad. Barras de error =
desvio estandar entre las R=20 realizaciones (va_stdev_between_realizations,
S_stdev_between_realizations).

Uso:
    MPLCONFIGDIR=/private/tmp/tp2_mplconfig .venv-mpl311/bin/python \
        python/plot_vicsek_va_vs_S_lowrho_to_2.py
"""
...
OUT_DIR = REPO_ROOT / "figures" / "vicsek_va_vs_S_lowrho_to_2_v1"
...
# Tonos de violeta, claro (densidad baja) -> oscuro (densidad alta), muy
# espaciados entre si para que las cuatro densidades se distingan de un
# vistazo (paleta secuencial 'Purples' de ColorBrewer, extremos incluidos).
RHO_COLOR = {
    "rho_1_over_3pi": "#dadaeb",
    "rho_1_over_2pi": "#9e9ac8",
    "rho_1_over_pi": "#6a51a3",
    "rho_2": "#3f007d",
}
...
def main() -> int:
    ...
    ax.grid(False)
    ax.legend(loc="lower right", frameon=False, title="Vicsek", title_fontsize=19)

    fig.tight_layout()
    out_path = OUT_DIR / "vicsek_va_vs_S_lowrho_to_2.png"
