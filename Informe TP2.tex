...
\caption{Configuraciones del modelo de Vicsek para $\rho = 2$ y $t = 2000$. Panel izquierdo: $\eta = 1$. Panel derecho: $\eta = 5$. El color codifica la orientación de la velocidad.}
\label{fig:1}
\end{figure}

La conectividad geométrica no debe confundirse con el orden direccional. El observable $S$, definido como la fracción de partículas de la componente conexa más grande, puede ser alto incluso cuando $v_a$ es bajo: un conjunto puede permanecer conectado espacialmente y, al mismo tiempo, tener direcciones desordenadas. El análisis específico de $S$ se presenta en la Sección~\ref{sec:clusters}.
...
\caption{Configuraciones del modelo de votante para $\rho = 2$ y $t = 2000$. Panel izquierdo: $\eta = 0.1$ y $v_a(t) = 0.973$. Panel derecho: $\eta = 1$ y $v_a(t) = 0.354$. El color codifica la orientación de la velocidad.}
\label{fig:4}
\end{figure}
